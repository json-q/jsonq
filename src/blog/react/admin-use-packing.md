...
![image](./assets/admin-use-packing/1740465689594-2pgbb4ud.png)

上传组件的封装思路：

![image](./assets/admin-use-packing/1740476578202-6g6awrqd.png)
...
const ProFormUploadDraggerToken: React.FC<ProFormUploadDraggerTokenProps> = (
  props
) => {
  ...
  const onRemoveFile = async (file: UploadFile<R<string>>) => {
    ...
    if (file.uid.indexOf("rc-") !== -1 && file.response?.code !== 200)
      return true;
...
export const getFileIdByUploadValue = (fileList?: UploadFile<R<string>>[]) => {
  ...
  const attachmentId = fileList.reduce((accu, curr) => {
    if (curr.response?.data && curr.status !== "error") {
      ...
      (curr.response?.data || curr.uid.indexOf("rc-") === -1) &&
      curr.status !== "error"
    ) {
 ...
 * @returns
 */
export const getFileListByFileData = (
  fileData: FileData[] = []
): UploadFile[] => {
