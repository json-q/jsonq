...
![image](./assets/nextjs-header-security/202504121611060.png)
...
![image](./assets/nextjs-header-security/202504121640412.png)
...
![image](./assets/nextjs-header-security/202504121647490.png)
...
![image](./assets/nextjs-header-security/202504121702769.png)

而负责身份认证和重定向的中间件，发现存在这个 header，就知道不需要干活了，直接放行

> 这里代码不完整，实际上并不是仅仅判断 `x-middleware-subrequest `，而是每次递归请求都会给这个字段的值加上一个 `middleware`，如果相同的 `middleware` 超过 5 次，就会响应一个 `x-middleware-next: 1`，表示跳过中间件

也就是说当请求重定向超过 5 次时，next 就认为这个是自家 CDN 造成的死循环，就会不走中间件，直接放行。

![image](./assets/nextjs-header-security/202504121707177.png)

好了，到这时候，就能很明显看出问题了，有没有安全漏洞呢？那可太有了。

因为 header 是非常轻松就可以伪造的，也就是说，只要有 `x-middleware-subrequest` 这个 header，后端就会傻乎乎的跳过中间件逻辑，直接放行，随意访问服务资源，什么 DDOS、缓存污染、Auth 认证等，那都不是事儿
...
第一次这个 pr 漏洞被 merge，是在 2021 年 10 月，直到 2025 年 03 月 1 日，被外部专家发现并告知 Vercel，Vercel 才开始重视起来。

![image](./assets/nextjs-header-security/202504121720905.png)

### 巨大的草台班子

Vercel 在 3 月 1 日 接到漏洞到真正发布补丁，历时 17 天

![image](./assets/nextjs-header-security/202504121724628.png)

而在这 17 天，Vercel 团队探讨这么久，想出的补救方式是什么呢？就是再加一个 header `x-middleware-subrequest-id`，我不禁 ？？？

![image](./assets/nextjs-header-security/202504121734410.png)

在重定向之前，Nextjs 会把生成的随机数保存在全局对象中，等到新请求回来时，拿全局对象的随机数和 header 中的值做对比，这样就确保外部用户无法伪造这个 header。(他简直是个超人.jpg)

![image](./assets/nextjs-header-security/202504121737461.png)
...
![image](./assets/nextjs-header-security/202504121745879.png)

如果说最开始的漏洞是失误造成的，那此次补丁就不得不让人怀疑 Vercel 程序员的脑洞了 😅

而如此牛逼的 Vercel，又顶着压力拖了一周才发布了新补丁

![image](./assets/nextjs-header-security/202504121756515.png)
