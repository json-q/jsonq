...
export default function SignIn() {
  ...
  if (status === 'authenticated' || status === 'loading') {
    return (
      <Avatar className="ml-1 hidden h-6 w-6 md:block">
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button className="ml-1 hidden md:block">Login</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>登录认证</DialogTitle>
          <DialogDescription>Github 认证之前，请先输入密码验证可用性</DialogDescription>
        </DialogHeader>
        <Form {...form}>
